...
#returns 1 if size1 > size2, -1 if size2 > size1, 0 if size1 == size2
...
def num_of_pawns(curr_state, agent_id):
    board = pawn_list_to_marks_array(curr_state)
    num_of_pawns = 0
    for i in range(3):
        for j in range(3):
            if board[i][j] == str(agent_id + 1):
                num_of_pawns += 1
    return num_of_pawns

def block_list(curr_state, agent_id):
    board = pawn_list_to_marks_array(curr_state)
    block_list = []
    for i in range(3):
        for j in range(3):
            if board[i][j] == str(agent_id + 1) and board[i][(j+1)%3] == str(agent_id + 1) and board[i][(j+2)%3] == " ":
                block_list.append((i, (j+2)%3))
            elif board[i][j] == str(agent_id + 1) and board[(i+1)%3][j] == str(agent_id + 1) and board[(i+2)%3][j] == " ":
                block_list.append(((i+2)%3, j))
    for i in range(3):
        if board[i][i] == str(agent_id + 1) and board[(i+1)%3][(i+1)%3] == str(agent_id + 1) and board[(i+2)%3][(i+2)%3] == " ":
            if ((i+2)%3, (i+2)%3) not in block_list:
                block_list.append(((i+2)%3, (i+2)%3))
        if board[i][2-i] == str(agent_id + 1) and board[(i+1)%3][(2-i-1)%3] == str(agent_id + 1) and board[(i+2)%3][(2-i-2)%3] == " ":
            if ((i+2)%3, (2-i-2)%3) not in block_list:
                block_list.append(((i+2)%3, (2-i-2)%3))
    return block_list
'''
...
class State:
    ...
    def insert_copy(self, new_state):
        ...
        self.true_turn = new_state.true_turn
    ...
    def get_neighbors(self):
        ...
        for i in range(9):
            for pawn in pawns:
                ...
                next_state.true_turn+=1
                neighbor_list.append((action, next_state))

        return neighbor_list
    
    def get_log(self):
        text = "turn:" + str(self.true_turn) + "\n"
        for i in range(3):
            for j in range(3):
                text += "pos" + str(i) + "-" + str(j) + ":"
                if np.array_equal(self.player1_pawns["S1"][0], np.array([i,j])) or np.array_equal(self.player1_pawns["S2"][0], np.array([i,j])):
                    text += "0:S,"
                if np.array_equal(self.player2_pawns["S1"][0], np.array([i,j])) or np.array_equal(self.player2_pawns["S2"][0], np.array([i,j])):
                    text += "1:S,"
                if np.array_equal(self.player1_pawns["M1"][0], np.array([i,j])) or np.array_equal(self.player1_pawns["M2"][0], np.array([i,j])):
                    text += "0:M,"
                if np.array_equal(self.player2_pawns["M1"][0], np.array([i,j])) or np.array_equal(self.player2_pawns["M2"][0], np.array([i,j])):
                    text += "1:M,"
                if np.array_equal(self.player1_pawns["B1"][0], np.array([i,j])) or np.array_equal(self.player1_pawns["B2"][0], np.array([i,j])):
                    text += "0:L,"
                if np.array_equal(self.player2_pawns["B1"][0], np.array([i,j])) or np.array_equal(self.player2_pawns["B2"][0], np.array([i,j])):
                    text += "1:L,"
                text += "\n"
        return text
